...
<h3><center>CADASTRAMENTO DE POSTO</center></h3>
    <form method="post" action="processarcadm.php" name="inserir"  >
		 ...
		 <label for="cod_posto">CÓDIGO DO POSTO</label>
				<input type="text" class="form-control" name="cod_posto" readonly=“true” id="cod_posto" placeholder="Insira a Cota">
          </div>
...
<div class="form-row">
	...
	<div class="form-group col-md-4">
			 <input type="button" value="VOLTAR" class="btn btn-primary mb-2" onclick="voltar()". href='abertura.php'
			  ...
			  <div class="form-group col-md-4">
			  <button id="btn" type="button">Clique aqui</button>
...
return false;
		...
		<script type="text/javascript">
		  google.load("jquery", "1.4.2");
			...
			<script type="text/javascript">
		$(function(){
			$('#cod_posto').change(function(){
				if( $(this).val() ) {
					$('#id_tipo').hide();
					$('.carregando').show();
					$.getJSON('sub_categorias_post.php?search=',{cod_posto: $(this).val(), ajax: 'true'}, function(j){
						var options = '<option value="">Escolha Subcategoria</option>';	
						for (var i = 0; i < j.length; i++) {
							options += '<option value="' + j[i].id + '">' + j[i].nome_sub_categoria + '</option>';
						}	
						$('#id_tipo').html(options).show();
						$('.carregando').hide();
					});
				} else {
					$('#id_tipo').html('<option value="">– Escolha Subcategoria –</option>');
				}
			});
		});
		</script>
