...
if(!empty($nome_cidade)){
    ...
    $limit = 1;
    $result_aluno = "SELECT * FROM cidade1 WHERE nome_cidade =:nome_cidade   LIMIT :limit";
    ...
    if($resultado_aluno->rowCount() != 0){
		...
		$array_valores['cod_cidade'] = $row_aluno['idcidade']; 
          $array_valores['pop'] = $row_aluno['pop']; 
    }else{
		$array_valores['cod_cidade'] = 'não encontrado';
